...
describe Serinette do
  ...
  context "a class method #get_output_path" do
    it "should exist" do
      Serinette.should respond_to :get_output_path
    end

    it "should get output path" do
      path = "#{Serinette::TMP_DIR}#{Serinette::OUTPUT_FILE_NAME}#{Serinette::SOUND_FILE_TYPE}"
      expect(Serinette.get_output_path).to eq path
    end
  end
  ...
  context "a class method #orchestrate" do
    ...
    it "should output a file" do
      ...
      expect(File.exist?(Serinette.get_output_path)).to be true
    end
  end
